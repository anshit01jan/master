# Claude Instructions: Software Development Lifecycle (SDLC) and Automation Framework Standards
...
- Every phase of the SDLC workflow is protected by a `PreToolUse` check before tool use and a `PostToolUse` check after tool use.
- Pretool hooks run before any tool call and validate inputs for credentials/secrets and malicious patterns.
- Posttool hooks log tool execution to the audit log and flag (without blocking) potential security violations in tool output.
...
- Automation results must always reflect the actual outcome of the most recent execution. Never overwrite or omit a failing result to make an artifact or PR look cleaner than the real run.
...
- Use the Playwright MCP server for getting locators and other UI elements for automation.

## Workflows and Outputs
- The `/run-sdlc-workflow` command loads these instructions once and shares them with every subagent it invokes.
- Subagents must not reread this file or their skill file mid-execution once context has been passed to them.
...
- Use `/run-sdlc-workflow` to unify instructions and results across phases.
...
- Do not allow subagents to independently reinterpret the instructions.
- Do not alter, hide, or replace a failing automation/test result to make a rerun or PR appear more successful than it was.
