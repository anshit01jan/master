...
argument-hint: "Automate all test cases from test_cases.md using Java hybrid framework"
---
...
Rerun note: when a later rerun succeeds, refresh automation artifacts so they reflect the latest run only.
...
7. Ensure implementation strictly follows .github/copilot-instructions.md for:
...
12. Validate that the final report reflects the latest run only.
13. Always use PlayWright MCP tool to navigate to base url and other web pages and capture all the necessary locators required for each web page and store them in the appropriate page object class.
...
4. agent-output/automation_test_results.md is updated with full counts, percentages, and Extent report URL/path.
5. Implementation aligns with .github/copilot-instructions.md.